...
def _evaluate_root_move(args):
    """Worker invoked by :mod:`multiprocessing` for a single root move.

    ``args`` is a tuple containing the information the child process needs:
    ``(state, move, depth, time_limit, piece, init_ff_branch_max,
    z_table, z_memory)``.  The zobrist table and memory are shared objects
    created by the parent so that all children can read/write the same cache.
    The helper makes a fresh :class:`MinimaxAgent` instance, plays the
    supplied move, and then calls :meth:`MinimaxAgent.minimax` with one less
    depth remaining.  Only the move, its value and the fast‑forward count
    are returned so that the parent process can choose the best option.
    """
    (
        state,
        move,
    ...
    # local agent copy, replicate a few fields used by minimax
    agent = MinimaxAgentParallel(state, piece, init_ff_branch_max)
    agent.eval_calls = 0
    agent.full_squares = set(
        [(i, j) for i in range(state.w) for j in range(state.h) if state.board[i][j] != game.EMPTY_PIECE]
    )
    agent.depth_counter = {}
    agent.depth_offset = depth

    # apply the candidate move and search one layer deeper
    new_state = state.make_move(move)
    z_hashing = (z_table, z_memory, 0)
    _, value, fff = agent.minimax(
        new_state,
        depth - 1,
        latest_time_limit,
        float("-inf"),
        ...
        z_hashing,
        ff=0,
        ff_branch_max=init_ff_branch_max,
    )
    return move, value, fff, agent.eval_calls, agent.depth_counter
...
class MinimaxAgentParallel(agent.Agent):
    ...
    def choose_move(self, state: game.GameState, time_limit: float):
        ...
        while depth <= max_depth:
            ...
            """Initialise Zobrist hash table (shared metadata for this iteration)"""
            ...
            try:
                # gather legal moves once and evaluate them in parallel
                legal_moves = [
                    (i, j)
                    for i in range(w)
                    for j in range(h)
                ...
                if len(legal_moves) <= 1:
                    # nothing to parallelise; fall back to serial call
                    move, value, fff = self.minimax(
                        state,
                        depth,
                        latest_time_limit,
                        float("-inf"),
                        ...
                        z_hashing,
                        ff=0,
                        ff_branch_max=self.init_ff_branch_max,
                    )
                else:
                    # create managed memory for shared zobrist cache
                    manager = multiprocessing.Manager()
                    z_memory = manager.dict()
                    work = [
                        (
                            state,
                            mv,
                    ...
                    with multiprocessing.Pool(processes=os.cpu_count()) as pool:
                        results = pool.map(_evaluate_root_move, work)

                    # select best result using same sign logic as later in loop
                    sign = 1 if self.piece == game.X_PIECE else -1
                    best_move = None
                    best_value = float("-inf") if sign == 1 else float("inf")
                    best_fff = 0
                    for mv, val, fff, e_calls, d_counter in results:
                        if sign * val > sign * best_value:
                            best_move, best_value, best_fff = mv, val, fff
                        self.eval_calls += e_calls
                        for d, c in d_counter.items():
                            if d in self.depth_counter:
                                self.depth_counter[d] += c
                            else:
                                self.depth_counter[d] = c
                    move, value, fff = best_move, best_value, best_fff
